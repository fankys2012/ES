...
use frame\base\InvalidParamException;
use frame\web\HeaderCollection;


class Response extends \frame\base\Response
{
    /**
    ...
    public function init()
    {
        ...
        if ($this->charset === null) {
            $this->charset = frame\Base::$app->charset;
        }
...
//        $this->trigger(self::EVENT_AFTER_SEND);
    ...
    protected function sendCookies()
    {
        ...
        $request = frame\Base::$app->getRequest();
        ...
        foreach ($this->getCookies() as $cookie) {
            ...
            if ($cookie->expire != 1  && isset($validationKey)) {
                $value = frame\Base::$app->getSecurity()->hashData(serialize([$cookie->name, $value]), $validationKey);
            }
    ...
    {
        return [
            self::FORMAT_HTML => 'frame\web\HtmlResponseFormatter',
//            self::FORMAT_XML => 'frame\web\XmlResponseFormatter',
            self::FORMAT_JSON => 'frame\web\JsonResponseFormatter',
            self::FORMAT_JSONP => [
                'class' => 'frame\web\JsonResponseFormatter',
                'useJsonp' => true,
            ],
        ];
    }
    ...
    protected function prepare()
    {
        ...
        if (isset($this->formatters[$this->format])) {
            ...
            if (!is_object($formatter)) {
                $this->formatters[$this->format] = $formatter = frame\Base::createObject($formatter);
            }
