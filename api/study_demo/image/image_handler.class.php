 ...
 * 图片处理类
 */
class nl_image_handler {

    var $maskAfterCut = false; //图片是否先打水印再剪切
    ...
    function setMaskAfterCut($bool){
        $this->maskAfterCut = $bool;
    }
...
//                若生成图片大小为0，则删除该图片 BY S67 20131230
    ...
    function _createMask() {
        ...
        if ($this->mask_img) {
            ...
            if ($this->_isFull() || !$this->maskAfterCut) {
        ...
        if (empty($this->mask_word) && empty($this->mask_img)) {
            ...
            //这里没有考虑gif的情况,目前是png
            imagealphablending($this->h_dst, false);//取消默认的混色模式（为解决阴影为绿色的问题）
            imagesavealpha($this->h_dst, true);//设定保存完整的 alpha 通道信息（为解决阴影为绿色的问题)
...
//                $this->_checkValid($mask_type);
        ...
        // $this->h_mask = imagecreatefrompng($this->mask_img);
        $this->h_mask = ImageCreateFromString($src);
//            imagesavealpha($this->h_mask,true);//这里很重要;
...
//                        图片生成容错 BY S67 20131230
    ...
    function _getImgType($file_path) {
        ...
        //if (file_exists($file_path)) { file_exists不能检测全路径地址文件
...
//        }
    ...
    public function cornerMark($src_img,$water_img)
    {
        $dst_info = getimagesize($src_img);
        $this->img_type = $dst_info[2];
        $this->src_w = $dst_info[0];
        $this->src_h = $dst_info[1];
        switch ($dst_info[2])
        {
            case 1:
                $dst_im =imagecreatefromgif($src_img);break;
            case 2:
                $dst_im =imagecreatefromjpeg($src_img);break;
            case 3:
                $dst_im =imagecreatefrompng($src_img);break;
            case 6:
                $dst_im =imagecreatefromwbmp($src_img);break;
            default:
                die("不支持的文件类型1");exit;
        }

        $src_info = getimagesize($water_img);
        $this->mask_w = $src_info[0];
        $this->mask_h = $src_info[1];
        switch ($src_info[2])
        {
            case 1:
                $src_im =imagecreatefromgif($water_img);break;
            case 2:
                $src_im =imagecreatefromjpeg($water_img);break;
            case 3:
                $src_im =imagecreatefrompng($water_img);break;
            case 6:
                $src_im =imagecreatefromwbmp($water_img);break;
            default:
                die("不支持的文件类型1");exit;
        }
        ...
        //支持png本身透明度的方式
        imagecopy($dst_im,$src_im,$this->mask_pos_x, $this->mask_pos_y,0,0,$src_info[0],$src_info[1]);
        ...
        switch ($dst_info[2]){
            case 1:
                imagegif($dst_im,$this->dst_img);break;
            case 2:
                imagejpeg($dst_im,$this->dst_img);break;
            case 3:
                imagepng($dst_im,$this->dst_img);break;
            case 6:
                imagewbmp($dst_im,$this->dst_img);break;
            default:
                die("不支持的文件类型2");exit;
        }
        imagedestroy($dst_im);
...
/*
  ...
  $t = new image_handler();
  //给图片增加水印
  $t->setSrcImg("./data/upimg/epgimg/0ab521b439564eeeac35afe94c759f010.jpg");
  $t->setDstImg("./tmp/1.jpg");
  //加水印的图片
  $t->setMaskImg("./data/upimg/epgimg/2d04cbbc8695586455cd9f3e85e55fb10.png");
  //水印位置
  $t->setMaskPosition(4);
  //水印透明度
  $t->setMaskImgPct(10);
  //图片加边框 大小,颜色
  $t->setDstImgBorder(1, "#dddddd");
  // 指定缩放比例
  $t->createImg(1150, 1115);
 * 
 */
?>